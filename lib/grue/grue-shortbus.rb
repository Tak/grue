...
module Grue
  class GrueShortBus < ShortBus::ShortBus
    # Constructor
    def initialize()
      super
      ...
      @channels = []
      @hooks = []
      hook_command( 'GRUE', ShortBus::XCHAT_PRI_NORM, method( :enable), '')
      hook_command( 'GRUEDUMP', ShortBus::XCHAT_PRI_NORM, method( :dump), '')
    ...
    def your_message(words, data)
      ...
      begin
        ...
        if (/^([^ ]+\s?grued )[^ ]+ link from/.match(words[1]) || !@channels.detect{ |item| item == channel }) then return ShortBus::XCHAT_EAT_NONE; end
